...
def define_model(trial):
  hidden_size = trial.suggest_int("hiddem_size", 256,512,256)
  num_layers = trial.suggest_int("num_layers", 2, 4)
  channels = trial.suggest_int("channels", 128,512,128)
  t_samples = trial.suggest_int("t_samples", 512, 2048, 512)
  model = LSTM(t_samples, hidden_size=hidden_size, num_layers=num_layers, channels=channels)
...
def objective(trial):
    ...
    train_dataset = TimeSeriesClassifierDataset(data_folder, train_idx, t_samples=t_samples)
    val_dataset = TimeSeriesClassifierDataset(data_folder, val_idx, t_samples=t_samples)
    ...
    trainer = ClassifierTrainer(model=model, optimizer=optimizer, criterion=loss_fn,
    ...
    for epoch in range(num_epochs):
        t_loss, t_acc = trainer.train_epoch(DEVICE, only_p=True)
        v_loss, v_acc = trainer.eval_epoch(DEVICE, only_p=True)
...
if __name__ == "__main__":
    try:
        ...
        study = optuna.create_study(study_name='lstm', storage='sqlite:////data/optuna/lstm.db')
    except:
        
        print("loading study")
        study = optuna.load_study(study_name='lstm', storage='sqlite:////data/optuna/lstm.db')

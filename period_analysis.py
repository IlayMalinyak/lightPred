...
def find_period(data, lags, prom=None, plot=True, method='first', name=''):
  ...
  if len(peaks):
    ...
    if plot:
      plt.plot(lags[peaks[i:]], data_filtered[peaks[i:]], 'x')
    plt.show()
    return lags[peaks[max_idx]], peaks[max_idx], data_filtered
  plt.show()
  return None, None, None

def analyze_lc(lc, plots=True, plot_peaks=False, day_cadence=0.020832, gt=None, plot_path=None):
  combined_data=None
  p_samples = int((lc.time.value[-1] - lc.time.value[0])//day_cadence)
  # print(p_samples, len(lc.time.value))  
  wvt = WaveletTransform.from_lightcurve(lc, period_samples=p_samples)
  ...
  min_p_idx = np.where(acf_lags > MINIMAL_PERIOD)[0][0]
  gwps, w_lags = wvt.gwps[min_p_idx:], wvt.period[min_p_idx:]
  gwps /= np.max(gwps)
  # if np.argmax(gwps) == 0:
  #   print('no period')
  #   return
  acf, acf_lags = acf[min_p_idx:], acf_lags[min_p_idx:]
  acf_period, period_idx, acf_filtered = find_period(acf, acf_lags, prom=0.005, plot=plot_peaks, name='ACF')
  acf_filtered = acf_filtered/np.max(acf_filtered) if acf_filtered is not None else None 

  # w_period, period_idx, w_filtered = find_period(gwps, w_lags, 0.01, plot=plot_peaks, name='Wavelet')

  if period_idx is not None:
    lags = acf_lags
    # print("acf period ", acf_period)
    # print("wavelet period ",w_period)
    if len(gwps) > len(acf_filtered):
      gwps = gwps[:len(acf_filtered)]
    combined_data = acf_filtered*gwps
    combined_period, period_idx, combined_data = find_period(combined_data, lags, prom=0.01, method='max', plot=plot_peaks, name='combined')
  ...
  if period_idx is None:
    # print("no period")
    return
#   print("combined period ", combined_period)
  if plots:
    title = 'lightcurve raw data inclination={:0.2f}'.format(gt[1]) if gt is not None else'lightcurve raw data'
    plt.plot(lc.time.value, lc.flux.value, color='black')
    plt.title(title)
    if plot_path is not None:
      plt.savefig(plot_path + "_raw.png")
    plt.show()
    plt.plot(lags, combined_data, color='black', label='combined')

    plt.plot(lags, gwps, '--', color='green', label='wavelet')
    if acf_filtered is not None:
      plt.plot(acf_lags, acf_filtered, '--',color='orange', label='ACF')
    if gt is not None:
      plt.axvline(x=gt[0], linestyle=':', color='magenta', label='ground truth period={:0.2f}'.format(gt[0]))
    plt.axvline(x=combined_period, linestyle=':', color='gray', label='period={:0.2f}'.format(combined_period))
    plt.legend()
    plt.title("processed lightcurve")
    if plot_path is not None:
      plt.savefig(plot_path + "_processed.png")
    plt.clf()

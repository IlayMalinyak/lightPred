...
def train_ddp(local_rank):
    ...
    optim_params = {"betas": (0.6101496277379665, 0.8556133514656037),
    "lr": 0.0014130030543473972,
    "weight_decay": 0.0004910765043873998}

    net_params = {"out_channels1":32,
    "out_channels2":16,
    "t_samples":1536, "dropout": 0.32880861620477114,}
    ...
    train_dataset = WaveletDataSet(data_folder, train_list[:4000], t_samples=net_params['t_samples'])
    val_dataset = WaveletDataSet(data_folder, val_list[:1000], t_samples=net_params['t_samples'])

    train_dataloader = DataLoader(train_dataset, batch_size=b_size, num_workers=int(os.environ["SLURM_CPUS_PER_TASK"]))
    val_dataloader = DataLoader(val_dataset, batch_size=b_size, num_workers=int(os.environ["SLURM_CPUS_PER_TASK"]))
    ...
    output_filename = f'{log_path}/exp{exp_num}/1dcnn.json'
...
if __name__ == '__main__':
    ...
    train_dataset = WaveletDataSet(data_folder, train_list, t_samples=net_params['t_samples'])
    val_dataset = WaveletDataSet(data_folder, val_list, t_samples=net_params['t_samples'])
